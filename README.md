...
- Never overwrites an existing file. When a destination file has the same name, size, and modified timestamp, it can delete the equivalent source copy after explicit confirmation. Items with same-name files that have different metadata are skipped with their linked companions while the remaining items continue.
- Stops at the first non-collision move failure and reports moved, deleted, skipped, failed, and unattempted files.
...
The installer is currently unsigned, so Windows SmartScreen may display a warning until code signing is configured.
